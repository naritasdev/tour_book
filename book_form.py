...
def add_request_from_db(request_name, request_email, request_date, request_requests): 
    with engine.connect() as conn:
        result = text("insert into request (name,email,date,requests) values ('{request_name}', '{request_email}', '{request_date}', '{request_requests}')".format(request_name,request_email,request_date,request_requests) )
        conn.execute(result)
